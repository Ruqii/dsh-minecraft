...
// One-life mode, for the `one_life` board. Dying in Minecraft does not stop
// anything -- you respawn on the spot and can carry on to a diamond -- so a
// rule that says "the run ends at the first death" is only a scoring
// convention unless something enforces it. This enforces it.
//
// Not a tool: whether a run is over is not one of the agent's decisions, the
// same way being filmed is not.
const ONE_LIFE = (process.env.MC_ONE_LIFE ?? '') === '1'
let deaths = 0
let milestonesAtDeath = null
let runOver = false
...
export async function connect({ host = '127.0.0.1', port = 25565, username = 'dsh_agent', version = '1.20.4' } = {}) {
  ...
  b.on('death', () => {
    deaths += 1
    if (deaths === 1) {
      // everHeld is cumulative, so at this instant it is exactly the set of
      // rungs reached before dying -- which is what the board scores.
      milestonesAtDeath = [...everHeld].sort()
    }
    if (ONE_LIFE && !runOver) {
      runOver = true
      note(`died -- one-life run is over. Reached: ${milestonesAtDeath.join(', ') || 'nothing'}`)
    } else {
      note('died and respawned')
    }
  })
  ...
  // Watching is the default. Most people want to see the thing play, and the
  // live page needs no ffmpeg, no Chrome and no file to go and find.
  const view = await recorder.startViewer(b)
  note(view.url !== null
    ? `watch live at ${view.url}${view.rebuilt ? ' (built the canvas module first)' : ''}`
    : `no live view: ${view.reason}`)
  ...
  if (dir !== null) {
    ...
    // Recording off is the default, and silence about it means a player
    // finishes a run, goes looking for a video, and finds none was ever made.
    note('not saving an mp4 -- set MC_RECORD_DIR to a folder to record one too')
  }
  return observe()
}
...
// Written continuously, not only at disconnect. A scoring run is far more
// likely to end by hitting its time limit than by the agent politely saying
// goodbye -- the easy-difficulty run was killed at 30 minutes and produced
// nothing at all, despite having reached an iron pickaxe. Frames are already
// on disk the same way, so a killed run leaves both halves recoverable.
let lastOutcomeWrite = 0
function writeOutcomeThrottled(b) {
  if (recorder.wanted() === null) return
  const now = Date.now()
  if (now - lastOutcomeWrite < 3000) return
  lastOutcomeWrite = now
  try { writeOutcome(b, null, { partial: true }) } catch { /* never break a tool call over bookkeeping */ }
}

function writeOutcome(b, recording, { partial = false } = {}) {
  ...
  const outcome = {
    ...
    deaths,
    // Required by the one_life board once anything has died: the score comes
    // from this rather than from the closing inventory, because a diamond mined
    // on a second life is not a one-life diamond.
    ...(milestonesAtDeath === null ? {} : { milestones_at_death: milestonesAtDeath }),
    one_life: ONE_LIFE,
    // On a partial write the mp4 does not exist yet; the frames do. run.sh muxes
    // them and fills this in when the run was killed rather than finished.
    video: recording?.video ?? (partial ? '' : ''),
    seed: process.env.MC_SEED ?? '',
    mc_version: b.version ?? '',
    ...(partial ? { partial: true, frames_dir: pathJoin(recorder.wanted() ?? '', 'frames') } : {}),
  }
  try {
    const dir = recorder.wanted()
...
// Reading the world after death is fine and useful; changing it is not. The
// run is over, and anything done now would not count anyway.
function requireAlive() {
  const b = requireBot()
  if (runOver) {
    throw new Error(
      `this run ended at your first death (one-life mode). Reached: ${(milestonesAtDeath ?? []).join(', ') || 'nothing'}. ` +
      'Nothing done now counts -- disconnect and report.',
    )
  }
  return b
}
...
async function walkTo(b, x, y, z, range, ms = WALK_TIMEOUT_MS) {
  ...
  let timer
  try {
    // A long walk is the other place damage goes unseen, and it is worse there:
    // nothing else is happening, so the bot simply walks until it dies.
    ...
    const outcome = await Promise.race([
      ...
      new Promise(resolve => {
        watch = setInterval(() => { if (b.health < healthBefore - 0.5) resolve(HURT) }, 400)
      }),
    ...
    if (outcome === HURT) {
      ...
      return {
        reached: false,
        took_damage: round(healthBefore - b.health),
        health: b.health,
        spent: spentWalking(b, carriedBefore),
        reason: `stopped after taking damage on the way -- health ${b.health}. Something is attacking, or the route is hurting the bot.`,
      }
    }
...
export async function dig({ x, y, z }) {
  ...
  // An agent is blind between tool calls: damage only surfaces on the next
  // mc_observe. A real run died to a skeleton while mining -- health went
  // 9, 6.5, 4, 1.5, 0 inside one window, and the agent never saw a hit. Damage
  // belongs in the result of the action it happened during.
  const healthBefore = b.health
  ...
  const hurt = healthBefore - b.health
  return {
    dug: target.name,
    ...(hurt > 0.01 ? { took_damage: round(hurt), health: b.health } : {}),
...
const inventoryOf = (b) => {
  const items = b.inventory.items().map(i => {
    everHeld.add(i.name)
    return { name: i.name, count: i.count }
  })
  writeOutcomeThrottled(b)
  return items
}
...
// Swing at something. It does not pick the target and it does not pick the
// weapon: mc_equip exists for the second, and deciding whether to fight at all
// is the whole survival question the board is really asking.
export async function attack({ target, swings = 1 }) {
  const b = requireAlive()
  const p = b.entity.position
  const REACH = 3.5
  const candidates = Object.values(b.entities)
    .filter(e => e !== b.entity && (e.name === target || e.username === target || e.displayName === target))
    .map(e => ({ e, d: e.position.distanceTo(p) }))
    .sort((a, c) => a.d - c.d)
  if (candidates.length === 0) {
    const near = Object.values(b.entities)
      .filter(e => e !== b.entity && e.position.distanceTo(p) < 24)
      .map(e => e.name ?? e.username ?? e.type)
    return { error: `nothing called "${target}" is nearby. Within 24 blocks: ${[...new Set(near)].join(', ') || 'nothing'}` }
  }
  const { e: victim, d } = candidates[0]
  // bot.attack does no range check of its own, so a swing at something six
  // blocks away looks like a hit and does nothing.
  if (d > REACH) {
    return { error: `the nearest ${target} is ${round(d)}m away and melee reach is about ${REACH}m -- walk closer, or it cannot be hit` }
  }
  const healthBefore = b.health
  let landed = 0
  for (let i = 0; i < Math.max(1, Math.min(swings, 10)); i++) {
    if (!b.entities[victim.id]) break            // already dead or despawned
    if (victim.position.distanceTo(b.entity.position) > REACH) break
    try { b.attack(victim) } catch { break }
    landed++
    // Attack cooldown: swinging faster than this in 1.9+ does almost nothing.
    await new Promise(r => setTimeout(r, 650))
  }
  const gone = b.entities[victim.id] === undefined
  const hurt = healthBefore - b.health
  return {
    target,
    swings: landed,
    killed: gone,
    ...(hurt > 0.01 ? { took_damage: round(hurt) } : {}),
    health: b.health,
    ...(gone ? {} : { still_there: true, distance: round(victim.position.distanceTo(b.entity.position)) }),
    note: 'a bare hand does very little damage; what is held decides the fight',
  }
}

// Eat something already carried. Restores whatever was held afterwards, for
// the same reason walking and placing do: eating is not a decision to stop
// holding a pickaxe.
export async function eat({ item }) {
  const b = requireAlive()
  const food = b.inventory.items().find(i => i.name === item)
  if (!food) return { error: `there is no ${item} in the inventory`, food: b.food }
  if (b.food >= 20) return { error: `food is already full (${b.food}/20), so ${item} cannot be eaten yet`, food: b.food }
  const heldBefore = b.heldItem
  const foodBefore = b.food
  try {
    await b.equip(food, 'hand')
    await b.consume()
  } catch (e) {
    return { error: `could not eat ${item}: ${e.message}`, food: b.food }
  }
  if (heldBefore && b.heldItem?.type !== heldBefore.type) {
    const again = b.inventory.items().find(i => i.type === heldBefore.type)
    if (again) { try { await b.equip(again, 'hand') } catch {} }
  }
  return { ate: item, food_before: foodBefore, food: b.food, health: b.health, holding: b.heldItem?.name ?? null, inventory: inventoryOf(b) }
}
