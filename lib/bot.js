  ...
  if (collected === 0) {
    ...
    if (drop) {
      try {
        ...
        await b.pathfinder.goto(new goals.GoalNear(d.x, d.y, d.z, 0))
        await new Promise(r => setTimeout(r, 900))
        collected = count() - before
      } catch {
        // Genuinely unreachable. Reported below with where it is, so the
        // agent can decide, rather than left to guess at a cause.
      }
      if (collected === 0) {
        const d = drop.position
