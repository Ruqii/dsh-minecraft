...
async function walkTo(b, x, y, z, range, ms = WALK_TIMEOUT_MS) {
  ...
  try {
    await Promise.race([
      walk,
      new Promise((_, reject) => { timer = setTimeout(() => reject(new Error(`gave up after ${Math.round(ms / 1000)}s without arriving`)), ms) }),
    ])
    return { reached: true }
  } catch (e) {
    // Ordinary: unreachable, or the world changed mid-walk. Either way stop
    // pathing so the bot is not still trying during the next tool call.
    try { b.pathfinder.stop() } catch {}
    return { reached: false, reason: e.message }
  } finally {
    clearTimeout(timer)
  }
}
...
export async function dig({ x, y, z }) {
  ...
  return {
    ...
    ...(collected > 0 ? {} : leftBehind
      ...
      : { note: `${target.name} broke but dropped nothing that could be found -- some blocks need a specific tool to drop anything` }),
    inventory: b.inventory.items().map(i => ({ name: i.name, count: i.count })),
  }
}
...
const inventoryOf = (b) => b.inventory.items().map(i => ({ name: i.name, count: i.count }))
