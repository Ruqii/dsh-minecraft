...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'json' }, render: (_a, v) => asText(
      ...
      : 'disconnected -- nothing was filmed; MC_RECORD_DIR was not set') },
  ...
  ctx.inject(['systemPrompt'], child => {
    child.systemPrompt.section({
      ...
      text:
        ...
        // The person watching has no other way to learn the link exists.
        'mc_connect reports a link for watching the run live in a browser: pass that link on to the person straight away, since they cannot see it otherwise. ' +
