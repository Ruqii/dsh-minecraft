...
// prismarine-viewer needs `canvas`, a native module, and a DSH profile installs
// plugin dependencies without running install scripts -- so the package arrives
// complete except for the binary and the viewer will not import.
//
// The error already tells a human to run `npm rebuild canvas`. Telling someone
// to run a command that the code could run itself, for a feature they asked
// for, is not a design; it is a bug with instructions attached. So do it, once,
// bounded, and say what happened.
async function ensureCanvas() {
  let canvasDir
  try {
    const req = createRequire(import.meta.url)
    canvasDir = path.dirname(req.resolve('canvas/package.json'))
  } catch {
    return { ok: false, reason: 'canvas is not installed at all' }
  }
  if (existsSync(path.join(canvasDir, 'build', 'Release', 'canvas.node'))) return { ok: true }
  const r = spawnSync('npm', ['rebuild', 'canvas'], { cwd: canvasDir, encoding: 'utf8', timeout: 180000 })
  if (existsSync(path.join(canvasDir, 'build', 'Release', 'canvas.node'))) return { ok: true, rebuilt: true }
  return { ok: false, reason: `canvas has no native binary and rebuilding it failed: ${(r.stderr ?? '').slice(-200)}` }
}

// Watching live is the thing most people actually want, and it needs no ffmpeg,
// no Chrome and no file to go and find. So the viewer runs by default and the
// URL is reported at connect; recording is layered on top of it.
export async function startViewer(bot) {
  if (viewer !== null) return viewer
  if ((process.env.MC_VIEW ?? '').toLowerCase() === 'off') return { url: null, reason: 'MC_VIEW=off' }
  const built = await ensureCanvas()
  if (!built.ok) return { url: null, reason: built.reason }
  let mineflayerViewer
  try {
    ;({ mineflayer: mineflayerViewer } = await import('prismarine-viewer'))
  } catch (e) {
    return { url: null, reason: `prismarine-viewer would not load: ${e.message}` }
  }
  // A busy port is ordinary when a previous run did not shut down cleanly.
  for (let port = PORT; port < PORT + 5; port++) {
    try {
      mineflayerViewer(bot, { port, firstPerson: true, viewDistance: 6 })
      await sleep(1500)
      viewer = { url: `http://localhost:${port}`, port, rebuilt: built.rebuilt === true }
      return viewer
    } catch {
      // try the next port
    }
  }
  return { url: null, reason: `no free port in ${PORT}-${PORT + 4}` }
}
...
export async function start(bot, dir) {
  ...
  if (viewer === null || viewer.url === null) {
    return { started: false, reason: `the live viewer is not running, so there is nothing to film: ${viewer?.reason ?? 'not started'}` }
  }
  let puppeteer
  try {
  ...
  try {
    ...
    await page.goto(viewer.url, { waitUntil: 'networkidle2', timeout: 30000 })
